...
def call() {
    ...
    if ! [ -z ${CI_MESSAGE} ]; then
        ...
        if ! [ -z $JOB_COMPOSE_COMPOSE_INFO_PAYLOAD_COMPOSE_ID ]; then
            ...
            if ! [[ ${JOB_BUILD_PACKAGE_NAME} =~ 'kernel' ]]; then
                python /home/ec2/mini_utils/html_parser.py --url "${BREW_BUILD_URL}${JOB_BUILD_BUILD_ID}" --dir "${WORKSPACE}" --keyword "${JOB_BUILD_PACKAGE_NAME}-\\d*.*(noarch|${DEFAULT_ARCH})" --excludekeys "src"
            else
                python /home/ec2/mini_utils/html_parser.py --url "${BREW_BUILD_URL}${JOB_BUILD_BUILD_ID}" --dir "${WORKSPACE}" --keyword "${JOB_BUILD_PACKAGE_NAME}-" --excludekeys "src,devel-,internal,debuginfo,extra,headers,cross,debug,tools,rt,uki,64k" --andkeys "${DEFAULT_ARCH}" --element a --field href     
            fi
            python /home/ec2/mini_utils/html_parser.py --url "${BREW_BUILD_URL}${JOB_BUILD_BUILD_ID}" --dir "${WORKSPACE}" --keyword "Tags" --element tr --field text --name BREWTAG -r
            source ${WORKSPACE}/job_env.txt
            PKG_URL=${JOB_PKGURL}
            compose_id=${JOB_BUILD_NVR}
        else
            baseurl=$JOB_LOCATION
        fi
        if [ -z $JOB_BUILD_BUILD_ID ]; then
            baseurl=${baseurl//'"'}
