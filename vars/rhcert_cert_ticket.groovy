...
def call() {
    ...
    out=$(rhcert_manager cert --classificationId ${CERT_CERT_CLASSIFICATIONID} --partnerProductId ${CERT_PRODUCT_ID}  --certificationTypeId ${CERT_CERT_CERTIFICATIONTYPEID} --content $content --new)
    if (( $? != 0 )); then
        echo $out
        exit 1
    fi
    ...
    rhcert_manager cert --id $id --list
