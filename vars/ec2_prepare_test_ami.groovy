...
def call() {
    ...
    if [[ $COMPOSE_ID =~ 'RHEL-7' ]]; then
        ...
        pkgs="make,automake,autoconf,sysstat,gcc,unzip,wget,quota,bzip2,iperf3,pciutils,fio,psmisc,expect,perf,nvme-cli,pciutils,fio,libaio-devel,blktrace,fio,nvme-cli,git,tar,nfs-utils,libvirt,qemu-kvm,python3,dracut-fips,kernel-debug,python3-pip,hostname,podman,xdp-tools,openssl-devel,glibc-all-langpacks,sos,strace,acpid,mokutil,libfabric,openmpi,kernel-modules-extra,iproute-tc,libcgroup,bcc-tools,libbpf,wireshark-cli,tcpdump,nmap-ncat,net-tools,tpm2-tools,tpm2-abrmd,acpica-tools,openmpi-devel,infiniband-diags,socat"
